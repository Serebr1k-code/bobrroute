...
export default function DefaultToolCard({
  ...
  const resolveApiKeyValue = useCallback(
    () => selectedKeyObj?.rawKey || (!cloudEnabled ? "sk_bobrroute" : t("yourApiKeyPlaceholder")),
  ...
  useEffect(() => {
    const savedModel = localStorage.getItem(`bobrroute-cli-model-${toolId}`);
    if (savedModel) {
      if (isMultiModelTool) {
        try {
    ...
    const savedKey = localStorage.getItem(`bobrroute-cli-key-${toolId}`);
  ...
  const handleModelChange = useCallback(
    (value) => {
      setModelValue(value);
      if (value) {
        localStorage.setItem(`bobrroute-cli-model-${toolId}`, value);
      } else {
        localStorage.removeItem(`bobrroute-cli-model-${toolId}`);
      }
    },
    [toolId]
  );
  ...
  const handleModelValuesChange = useCallback(
    (values) => {
      ...
      if (normalized.length > 0) {
        localStorage.setItem(`bobrroute-cli-model-${toolId}`, JSON.stringify(normalized));
      } else {
        localStorage.removeItem(`bobrroute-cli-model-${toolId}`);
      }
    },
  ...
  const handleApiKeyChange = useCallback(
    (value) => {
      ...
      if (value) {
        ...
        localStorage.setItem(`bobrroute-cli-key-${toolId}`, value);
      }
    },
    [toolId]
  );
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/cli-tools/guide-settings/${toolId}`, {
        ...
        body: JSON.stringify({
          baseUrl: baseUrlWithV1,
          apiKey: !cloudEnabled ? "sk_bobrroute" : null,
          keyId: selectedKeyId,
          model: modelValue,
      ...
      <div className="mt-2 flex items-center gap-2">
        {apiKeys && apiKeys.length > 0 ? (
          ...
          <span className="text-sm text-text-muted">
            {cloudEnabled ? t("noApiKeysCreateOne") : "sk_bobrroute"}
          </span>
        )}
      </div>
    );
