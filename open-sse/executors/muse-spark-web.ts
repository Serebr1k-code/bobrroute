...
function buildStreamingResponse(
  ...
  id: string,
  created: number,
  toolCalls?: Array<{ id: string; type: string; function: Record<string, unknown> }>
      ...
      controller.enqueue(
        encoder.encode(
          sseChunk({
            ...
            model,
            system_fingerprint: null,
            choices: [{ index: 0, delta: {}, finish_reason: toolCalls && toolCalls.length > 0 ? "tool_calls" : "stop", logprobs: null }],
          })
        )
      );
...
function buildSuccessResult(
  parsed: ParsedMetaAiResponse & { toolCalls?: Array<{ id: string; type: string; function: Record<string, unknown> }> },
  stream: boolean,
  model: string,
  headers: Record<string, string>,
  transformedBody: unknown
  ...
  const response = stream
    ? new Response(buildStreamingResponse(deltas, reasoningDeltas, model, id, created, toolCalls), {
        status: 200,
        headers: {
    ...
    : buildNonStreamingResponse(parsed.content, parsed.reasoningContent, model, id, created, toolCalls);
...
export class MuseSparkWebExecutor extends BaseExecutor {
  ...
  async execute({
    ...
    // Inject tool definitions into messages if tools are present
    let processedMessages = messages;
    if (tools && tools.length > 0) {
      processedMessages = injectToolsIntoMessages(messages, tools);
    }

    const parsedHistory = parseOpenAIMessages(processedMessages);
    ...
    // Process tool calls if tools were provided
    if (tools && tools.length > 0) {
      const { contentWithoutTools, toolCalls } = processScaffoldedToolResponse(parsed.content);
      
      // If tool calls were found, add them to the parsed response
      if (toolCalls.length > 0) {
        (parsed as any).toolCalls = convertToolCallsToOpenAIFormat(toolCalls);
        parsed.content = contentWithoutTools;
      }
    }
